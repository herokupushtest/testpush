...
function initScroll(elementId) {
	...
	if (displacement >= totalDistance) {
		...
		console.log(totalDistance, displacement);
...
document.getElementById('submitbtn').addEventListener('click', (e) => {
	...
	console.log('element is ', document.getElementById('screenName'));

	console.log('text is', screennameText.value);

	document.getElementById('submitbtn').disabled = true;



	fetch(`http://localhost:2400/userInfo/${screennameText.value}`)
	    .then((res) => res.json())
	    .then((res) => {
			userInfo = res;
...
function getInitialData(screenName) {
	console.log('work');
	fetch(`http://localhost:2400/followers/${screenName}`)
	    .then((res) => res.json())
	    .then((res) => {
	      console.log(res);
	      data = res;
...
function initMap() {
    ...
    console.log('data shama is', data);
    for (user of data) {
...
function getFollowing() {
	...
	if (!fetching) {
		...
		fetch(`http://localhost:2400/following/${userInfo["screenName"]}`)
		  .then((res) => res.json())
		  .then((res) => {
		    data = res;
		    console.log(data);
...
function getFollowers() {
	...
	if (!fetching) {	
		...
		fetch(`http://localhost:2400/followers/${userInfo["screenName"]}`)
		  .then((res) => res.json())
		  .then((res) => {
		    data = res;
		    console.log(data);
